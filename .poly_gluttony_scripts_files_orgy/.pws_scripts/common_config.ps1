...
function Get-ToolPaths {
    ...
    return @(
        ...
        "$Root\.computer_languages\mingw64\bin"                # MinGW64 GCC Toolchain v13.2.0
        "$Root\.computer_languages\msys64\usr\bin"             # MSYS2 Pacman v6.1.0 & Unix Tools
        "$Root\.computer_languages\msys64\ucrt64\bin"          # MSYS2 UCRT64 Toolchain (includes make)
