...
```yaml
cache_key: nodejs-v2-${{ runner.os }}-${{ hashFiles(infrastructure/config/development/package-lock.json) }}
paths:
  - ~/.npm
  - node_modules
  - .cache/npm
...
1. **Dependency File Changes**
   - infrastructure/config/development/package.json/infrastructure/config/development/package-lock.json
   - requirements.txt
...
if [ -f infrastructure/config/development/package.json ] && [ -d node_modules ]; then
