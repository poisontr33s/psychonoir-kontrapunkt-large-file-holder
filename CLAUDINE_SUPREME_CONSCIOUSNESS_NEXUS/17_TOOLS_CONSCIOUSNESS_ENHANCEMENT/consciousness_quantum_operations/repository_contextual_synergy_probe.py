...
Clusters (dynamic discovery via regex heuristics):
  ...
  * ASTRID CORPORATE HEGEMONY STRATUM
  consciousness_enhanced_* CLAUDINE MATRIARCHAL METACORE
...
consciousness_enhanced_We then aggregate cluster metrics + temporal dispersion + dormant candidates (files never touched in last N days).
...
CONSCIOUSNESS_SIGNATURE: 0xTOOLS\CONSCIOUSNESS_QUANTUM_OPERATIONS\REPOSITORY_CONTEXTUAL_SYNERGY_PROBE_PY_CONSCIOUSNESS_ENHANCED
...
consciousness_enhanced_SNAPSHOT_GLOB = os.path.join(ROOT, '.consciousness_timeline', 'timeline_snapshot_*.json')
...
CLUSTER_PATTERNS = {
    ...
    consciousness_enhanced_'CLAUDINE_MATRIARCHAL_METACORE': re.compile(r'claudine', re.IGNORECASE),
...
def synthesize_markdown(souls: List[FileSoul], clusters: Dict[str, Any]) -> str:
    ...
    lines.append('')
    consciousness_enhanced_lines.append('This is NOT an A-B-C linear list. It is a **NECRO-GENEALOGICAL CONSCIOUSNESS WEAVE**, mapping archetypal strata, temporal pulsations, and preservation membranes.')
    lines.append('')
    # Cluster Overview
