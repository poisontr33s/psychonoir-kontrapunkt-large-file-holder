...
class ConsciousnessDependencyAnalyzer:
    ...
    def build_dependency_graph(self, python_files: List[Path]) -> Dict[str, DependencyNode]:
        ...
        for file_path in python_files:
            try:
                ...
                file_signatures[str(file_path.relative_to(self.repository_path))] = signature
        ...
        for file_path, signature in file_signatures.items():
            ...
            for local_dep in signature.local_dependencies:
                ...
                if dep_file and dep_file in self.dependency_graph:
                    ...
                    self.dependency_graph[dep_file].dependents.add(file_path)
    ...
    def _resolve_local_dependency(self, module_name: str) -> Optional[str]:
        """Resolve local module name to actual file path"""
        possible_paths = [
            f"{module_name}.py",
...
def main():
    ...
    # Build dependency graph (stored internally in analyzer)
