...
# Bun configurations
if [ -f infrastructure/config/development/bunfig.toml ]; then
    cp infrastructure/config/development/bunfig.toml "$CRITICAL_BACKUP_DIR/bunfig-$(date +%Y%m%d-%H%M%S).toml"
    echo "✅ Bun config backed up"
fi

# Package.json og archives/legacy/old_scripts/bun.lock
if [ -f infrastructure/config/development/package.json ]; then
    cp infrastructure/config/development/package.json "$CRITICAL_BACKUP_DIR/package-$(date +%Y%m%d-%H%M%S).json"
    echo "✅ Package.json backed up"
fi

if [ -f archives/legacy/old_scripts/bun.lock ]; then
    cp archives/legacy/old_scripts/bun.lock "$CRITICAL_BACKUP_DIR/bun-lock-$(date +%Y%m%d-%H%M%S)"
...
CRITICAL_FILES=(
    ".vscode/mcp.json"
    infrastructure/config/development/bunfig.toml
    infrastructure/config/development/package.json
    archives/legacy/old_scripts/bun.lock
)
