...
def organize_root_files():
    ...
    # Load analysis data (optional, with error handling)
    try:
        with open("ROOT_INFRASTRUCTURE_ANALYSIS.json", "r", encoding="utf-8") as f:
            json.load(f)  # Loaded for validation, not used
    except FileNotFoundError:
        print("⚠️ ROOT_INFRASTRUCTURE_ANALYSIS.json not found. Proceeding without analysis data.")
    except json.JSONDecodeError:
        print("⚠️ ROOT_INFRASTRUCTURE_ANALYSIS.json is malformed. Proceeding without analysis data.")
