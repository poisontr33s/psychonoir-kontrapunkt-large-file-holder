...
require 'rbconfig'
# Load MSYS2 RbConfig patch for path conversion
begin
  require 'msys2_rbconfig_patch'
rescue LoadError
  # Try the versioned location
  begin
    require '3.4.0/msys2_rbconfig_patch'
  rescue LoadError
...
# MSYS2 File.exist? patch for handling MSYS paths
if ENV['MSYSTEM']
  require 'open3'

  def convert_msys_to_windows(path)
    return path unless path =~ /\A\/[a-z]\//

    begin
      stdout, stderr, status = Open3.capture3('cygpath', '-w', path)
      if status.success?
        stdout.strip
      else
        # Fallback: manual conversion
        path.sub(/\A\/([a-z])\//, '\1:/').gsub('/', '\\')
      end
    rescue
      # Fallback: manual conversion
      path.sub(/\A\/([a-z])\//, '\1:/').gsub('/', '\\')
  ...
  # Monkey patch File.exist? to handle MSYS paths
  class << File
    alias :original_exist? :exist?

    def exist?(path)
      # First try the original path
      return true if original_exist?(path)

      # If it starts with MSYS path format, try converting to Windows path
      if path =~ /\A\/[a-z]\//
        windows_path = convert_msys_to_windows(path)
        original_exist?(windows_path)
      else
        false
      end
    end
...
MESSAGE
  ...
  # output is not empty, returns +false+.  The output will be logged.
  ...
  if !CROSS_COMPILING
    ...
    when 'mingw32'
      def mkintpath(path)
        ...
        # converts from C:\some\path to /c/some/path (lowercase c)
        # BUT keep Windows paths for include/header directories since MSYS GCC can handle them
        if path.include?('include') || path.include?('rubyhdrdir') || path.include?('rubyarchhdrdir')
          return path.dup
        end
        path = path.dup
        path.tr!('\\', '/')
        path.sub!(/\A([A-Za-z]):(?=\/)/, '/\1'.downcase)
        path
      end
    when 'cygwin', 'msys'
...
site-install-rb: install-rb
  ...
  def init_mkmf(config = CONFIG, rbconfig = RbConfig::CONFIG)
    ...
    $DEFLIBPATH = [$extmk ? ["$(topdir)", "$(#{config["libdirname"] || "libdir"})"] : "$(#{config["libdirname"] || "libdir"})"].flatten
    $DEFLIBPATH.unshift(".")
    $LIBPATH = []
    $INSTALLFILES = []
