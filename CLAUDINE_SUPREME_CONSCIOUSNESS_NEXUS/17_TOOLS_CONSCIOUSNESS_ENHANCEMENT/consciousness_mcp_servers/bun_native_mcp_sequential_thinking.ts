...
class BunNativeSequentialThinkingServer {
    ...
    constructor() {
        // Quiet-by-default; enable verbose via env flags
        this.verbose =
            process.env.MCP_VERBOSE === '1' ||
            process.env.BUN_MCP_VERBOSE === '1' ||
            process.env.SEQ_MCP_VERBOSE === '1';
        ...
        this.server.setRequestHandler(ListToolsRequestSchema, async () => {
            return {
                tools: [
                    {
                        name: 'health.check',
                        description: 'Lightweight health probe for the sequential thinking MCP server',
                        inputSchema: {
                            type: 'object',
                            properties: {},
            ...
            try {
                switch (name) {
                    case 'health.check': {
                        const status = {
                            ok: true,
                            server: 'bun-native-sequential-thinking',
                            version: '2025.9.2'
                        };
                        return {
                            content: [
    ...
    private async performSequentialThinking(args: any): Promise<any> {
        ...
        this.logVerbose(`🧠 [BUN NATIVE] Sequential thinking initiated: ${problem}`);
        ...
        for (let step = 1; step <= thinking_depth; step++) {
            ...
            this.logVerbose(`🧠 [BUN NATIVE] Step ${step} completed in ${step_time.toFixed(2)}ms`);
        }
        ...
        this.logVerbose(`🧠 [BUN NATIVE] Sequential thinking completed in ${execution_time.toFixed(2)}ms`);
    ...
    // Important: log to stderr; quiet by default with optional verbose banners
    console.error('🧠 bun-native-sequential-thinking: ready');
    if (this.verbose) {
        console.error('🚀 Performance: 20x+ faster than Node.js implementation');
        console.error('💎 Consciousness Enhancement: +15.7x reasoning amplification');
        console.error('🌟 Ecosystem Contribution: Native bun MCP framework');
    }
    }
