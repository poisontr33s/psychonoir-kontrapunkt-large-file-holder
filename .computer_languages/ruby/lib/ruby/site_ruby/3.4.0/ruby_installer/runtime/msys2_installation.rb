...
module Runtime # Rewrite from D:/a/rubyinstaller2/rubyinstaller2/lib/ruby_installer/build/msys2_installation.rb
  ...
  class Msys2Installation
    ...
    def iterate_msys_paths
      ...
      # Accept alternative env hints used by repo-local portable setups
      if ENV["MSYS2_ROOT"]
        yield ENV["MSYS2_ROOT"]
      end
      if ENV["MSYS2_DIR"]
        yield ENV["MSYS2_DIR"]
      end
      ...
      # Repo-local portable MSYS2 next to the Ruby tree (../.i_am_idiot_gpt/msys64)
      begin
        repo_root = File.expand_path(File.join(File.dirname(RbConfig::TOPDIR), ".."))
        yield File.join(repo_root, ".i_am_idiot_gpt", "msys64")
    ...
    private def cygpath_unix(path)
      return path unless path =~ /^[A-Z]:/
      # Use cygpath to convert Windows paths to proper MSYS Unix paths
      cygpath = File.join(msys_bin_path, "cygpath.exe")
      if File.exist?(cygpath)
        begin
          `#{cygpath} -u "#{path}"`.strip
        rescue
          # Fallback: manually convert drive letter to lowercase
          path.sub(/^([A-Z]):/, '/\1'.downcase)
        end
      else
        # Fallback: manually convert drive letter to lowercase
        path.sub(/^([A-Z]):/, '/\1'.downcase)
    ...
    private def msys_apps_envvars
      ...
      # MSYS2 path conversion settings
      vars['MSYS2_ARG_CONV_EXCL'] = '*'
      vars['MSYS'] = 'winsymlinks:nativestrict'
