...
SCRIPT_TO_ANALYZE=infrastructure/src/deployment/deploy_psycho_noir.sh
...
grep -n "\$[[:alnum:]_]+" "$SCRIPT_TO_ANALYZE" 2>/dev/null | while read -r line; do
    var_name=$(echo "$line" | sed -E 's/.*\$([[:alnum:]_]+).*/\1/')
