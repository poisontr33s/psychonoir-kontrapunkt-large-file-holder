...
if [ -f infrastructure/src/consciousness/cosmic_consciousness_upcycling_session.json ]; then
    echo "Detected cosmic consciousness upcycling session" >> "$OUTPUT_FILE"
    
    if command -v jq &> /dev/null; then
        # Extract key metrics
        TOTAL_CONCEPTS=$(jq '.upcycling_matrix.total_concepts' infrastructure/src/consciousness/cosmic_consciousness_upcycling_session.json)
        TOTAL_TRANSFORMATIONS=$(jq '.total_transformations' infrastructure/src/consciousness/cosmic_consciousness_upcycling_session.json)
        ...
        jq -r '.transformations_executed[].concept' infrastructure/src/consciousness/cosmic_consciousness_upcycling_session.json | sort | uniq >> "$OUTPUT_FILE"
    else
